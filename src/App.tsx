...
import { auth, db, googleProvider, signInWithPopup, signInWithRedirect, signOut, handleFirestoreError, OperationType } from './firebase';
  ...
  const handleGoogleLoginPopup = async () => {
    ...
    try {
      ...
      if (err?.code === 'auth/popup-blocked') {
        setAuthError("Popup blocked by your browser. Please allow popups or use the Redirect option below.");
      } else if (err?.code === 'auth/popup-closed-by-user') {
        setAuthError("Login window was closed before completing.");
      } else {
        setAuthError(err?.message || "Popup sign-in failed. Please try the Redirect method.");
      }
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col justify-between font-sans selection:bg-indigo-650 selection:text-white antialiased">
        ...
        <header className="bg-white border-b border-slate-100 py-5 px-6">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <strong className="text-slate-800 text-sm font-black block leading-none font-sans uppercase tracking-wider">VoiceOver Studio</strong>
                <span className="text-[9px] text-indigo-600 font-bold tracking-wider uppercase block">Co-Created with Google AI Studio</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center w-full max-w-5xl">
            ...
            <div className="lg:col-span-7 space-y-6">
              <span className="inline-flex items-center gap-1.5 bg-indigo-50 text-indigo-700 text-[10px] font-extrabold uppercase py-1 px-3 rounded-full border border-indigo-120 tracking-wider">
                ...
                Designed by Google AI Studio for Sonali Mahajan
              </span>
            ...
            <div className="lg:col-span-5 flex justify-center">
              <div className="bg-white rounded-3xl border border-slate-105 shadow-xl p-8 w-full max-w-sm space-y-6 text-center">
                ...
                <div className="space-y-3">
                  {/* Primary Option: Popup-based oauth */}
                  <button
                    onClick={handleGoogleLoginPopup}
                    id="btn-google-sign-in-popup"
                    className="w-full flex items-center justify-center gap-3 bg-indigo-600 hover:bg-indigo-750 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-sm active:scale-98 cursor-pointer text-xs"
                  >
                  ...
                  <div className="relative flex py-1 items-center justify-center">
                    ...
                    <span className="flex-shrink mx-3 text-[9px] text-slate-450 font-black tracking-widest uppercase">Can't work? Use</span>
                    <div className="flex-grow border-t border-slate-100"></div>
                  </div>

                  {/* Secondary Option: Direct Redirect fallback */}
                  <button
                    onClick={handleGoogleLoginRedirect}
                    id="btn-google-sign-in-redirect"
                    className="w-full flex items-center justify-center gap-3 bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-sm active:scale-98 cursor-pointer text-xs"
                  >
                    <svg className="w-4 h-4 text-white shrink-0" viewBox="0 0 24 24">
                      <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                      <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                ...
                {authError && (
                  <div className="bg-amber-50 border border-amber-200 text-[10px] text-amber-800 rounded-xl p-2.5 text-left leading-relaxed font-sans">
                    <strong>Sign-In Hint:</strong> {authError}
                  </div>
                )}
        ...
        <footer className="bg-white border-t border-slate-100 py-4 text-center text-xs text-slate-400">
          <p>© 2026 VoiceOver Studio. Designed by Google AI Studio for Sonali Mahajan. Engineered with verified Firebase integration.</p>
        </footer>
      </div>
    );
  }
      ...
      <footer className="bg-slate-900 text-white border-t border-slate-800/80 py-8 px-6 mt-16 text-xs text-center">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-400">Co-Created with Google AI Studio</span>
          </div>
          <p className="text-slate-500">© 2026 Designed for Sonali Mahajan. All rights reserved. Sandboxed via Antigravity Secure Cloud Engine.</p>
        </div>
      </footer>
      ...
      {showGoogleLoginModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-xs p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm border border-slate-100 overflow-hidden relative p-8 scale-100 transition-transform">
            ...
            {/* Real Firebase Authentication buttons within modal */}
            <div className="space-y-2 mb-4 bg-slate-50/50 p-3 rounded-2xl border border-slate-100">
              <span className="text-[9px] uppercase font-black tracking-widest text-indigo-600 block text-left mb-1 font-sans">
                Real Google Sign-In (Official Core):
              </span>
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={handleGoogleLoginPopup}
                  id="btn-modal-login-real-popup"
                  className="flex items-center justify-center gap-1.5 bg-indigo-600 hover:bg-indigo-750 text-white font-extrabold py-2 px-2.5 rounded-xl transition-all shadow-xs text-[10px] cursor-pointer"
                >
                  Popup Mode
                </button>
                <button
                  onClick={handleGoogleLoginRedirect}
                  id="btn-modal-login-real-redirect"
                  className="flex items-center justify-center gap-1.5 bg-slate-900 hover:bg-slate-800 text-white font-extrabold py-2 px-2.5 rounded-xl transition-all shadow-xs text-[10px] cursor-pointer"
                >
                  Redirect Mode
                </button>
              </div>
              {authError && (
                <div className="bg-amber-50 border border-amber-200 text-[9px] text-amber-800 rounded-lg p-2 text-left leading-normal mt-1">
                  <strong>Hint:</strong> {authError}
                </div>
              )}
            </div>
            ...
            <div className="relative flex py-2 items-center justify-center">
              <div className="flex-grow border-t border-slate-100"></div>
              <span className="flex-shrink mx-2.5 text-[8px] text-slate-400 font-black tracking-widest uppercase">Or Simulation Accounts</span>
              <div className="flex-grow border-t border-slate-100"></div>
            </div>
